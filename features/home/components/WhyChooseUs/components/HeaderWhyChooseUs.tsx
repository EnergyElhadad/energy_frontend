import React from "react";

const HeaderWhyChooseUs = () => {
  return (
    <div className="mb-8 text-center">
      <h2
        id="why-choose-us-heading "
        className="text-WetGray mb-4 text-base font-bold md:text-[1.5rem]"
      >
        لماذا تختار ENERGY ELHADAD؟
      </h2>
      <p className="text-WetGray mx-auto mb-4 max-w-202.5 text-[14px] leading-relaxed font-medium md:text-base">
        هي شركة متخصصة في توفير وبيع المنتجات والحلول الكهربائية عالية الجودة،
        حيث نعمل على تلبية احتياجات الأفراد والشركات من خلال مجموعة متكاملة من
        المنتجات الموثوقة التي تجمع بين الكفاءة، الأمان، والأداء العالي
      </p>
    </div>
  );
};

export default HeaderWhyChooseUs;
