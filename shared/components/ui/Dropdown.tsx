...
import { ReactElement, useState, useRef, useEffect, cloneElement, HTMLAttributes } from "react";
...
export const Dropdown = ({
  ...
  useEffect(() => {
    if (isOpen && menuRef.current && containerRef.current) {
      ...
      if (rect.right > viewportWidth - padding) {
        ...
        const finalWidth = rect.width > availableWidth ? availableWidth : rect.width;
        const newRelativeLeft = (viewportWidth - padding - finalWidth) - containerRect.left;
  ...
  const getAlignmentClasses = () => {
    ...
    if (menuAlign === 'center') return 'start-1/2 -translate-x-1/2 rtl:translate-x-1/2';
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <>
          {showArrow && (
            <div className="arrow-up w-[20px] absolute top-[43px] left-1/2 -translate-x-1/2 border-[6px] border-transparent border-b-white z-50"></div>
          )}
          <div
            ref={menuRef}
            className={`absolute top-[55px] w-[calc(100vw-32px)] max-w-[370px] bg-white rounded-[12px] shadow-[0px_4px_20px_0px_rgba(0,0,0,0.1)] z-50 p-4 sm:p-5 cursor-default sm:w-[370px] ${getAlignmentClasses()} ${menuClassName || ''}`}
          >
            {children}
          </div>
        </>
