    ...
    <section className="bg-Background py-10" aria-label="Featured Products">
      <div className="container">
        <div className="grid grid-cols-1 place-items-center gap-6 lg:grid-cols-2">
          {banners.map(banner => (
            <BannerCard key={banner.id} title={banner.title} descritption={banner.image_text} textLink={banner.button} urlImage={banner.image} mobileUrlImage={banner.mobile_image} link={banner.link} />
          ))}
        </div>
      </div>
    </section>
