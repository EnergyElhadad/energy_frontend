...
export const MainBannerContent: React.FC<MainBannerContentProps> = ({ title, desc1, desc2, linkUrl, linkText }) => {
  return (
    <div className="absolute top-1/2 left-1/2 z-10 w-full -translate-x-1/2 -translate-y-1/2 space-y-2 px-4 py-2 text-center">
      ...
      {desc1 && <p className="mb-4 text-2xl font-bold text-white md:text-[32px]">{desc1}</p>}
      {desc2 && <p className="mb-10 text-sm font-normal text-white md:text-base">{desc2}</p>}
      {linkText && (
