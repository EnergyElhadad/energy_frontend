...
export const AuthLinksDropdown = ({ direction }: { direction?: 'up' | 'down' }) => {
    ...
    <Dropdown
      trigger={
        <button className="group border-Stroke flex h-[42px] w-[127px] cursor-pointer items-center justify-center gap-[8px] rounded-full border px-[12px] py-[8px] transition-all duration-300 ease-in-out">
          ...
          <span className="text-WetGray text-[12px] font-semibold group-aria-expanded:text-white">{t('signin')}</span>
        </button>
      }
      triggerMode="click"
