...
export const CategoriesDropdown = ({ initialData }: { initialData: CategoriesResponse | null }) => {
  const t = useTranslations('Header');
  const { categories, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteCategories(initialData);
  ...
  return (
    <Dropdown
      trigger={
        <button className="flex items-center gap-[10px] transition-colors duration-300 ease-in-out">
          ...
          <span className="text-primary text-[14px]">{t('all_categories')}</span>
        </button>
      }
      triggerMode="hover"
      menuAlign="start"
    ...
    >
      <div className="max-h-[300px] overflow-y-auto">
        ...
        {hasNextPage && (
          <div className="p-2">
            <Button variant="outline" size="sm" className="w-full" onClick={() => fetchNextPage()} disabled={isFetchingNextPage}>
              {isFetchingNextPage ? <Spinner /> : t('show_more')}
            </Button>
          </div>
        )}
      </div>
